...
def fetch_and_store_data(
    ...
    while fetch_data:
        try:
                ...
                if model_class == EarthRangerEvents:
                    for feature in data['data']['results']:
                        ...
                        try:
                            ...
                            event, created = model_class.objects.update_or_create(
            ...
            logging.error(f"Unexpected error while fetching {model_class} data: {e}")
            fetch_data = False
...
def get_grouped_settings():
    ...
    settings = EarthRangerSetting.objects.filter(is_active=True).select_related('user')
...
def fetch_all_earth_ranger_data():
    ...
    # fetch for default EarthRanger first
    fetch_earth_ranger_events()
